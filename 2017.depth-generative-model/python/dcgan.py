...
parser.add_argument('--dataset', default='nyudepthv2', help='nyudepthv2 | cifar10 | lsun | imagenet | folder | lfw | fake')
parser.add_argument('--modality', default='rgb', help='rgb | rgbd')
parser.add_argument('--dataroot', default='data/nyudepthv2', help='path to dataset')
...
try:
    opt.outf = os.path.join('results', '{}.modality={}.nz={}.ngf={}.bs={}'
        .format(opt.dataset, opt.modality, opt.nz, opt.ngf, opt.batchSize))
...
elif opt.dataset == 'nyudepthv2':
    # assuming square images
    traindir = os.path.join(os.path.join('data', 'nyudepthv2'), 'train')
    dataset = nyu_dset.ImageFolder(traindir, type='train', modality=opt.modality)
    nc = len(opt.modality)
assert dataset
