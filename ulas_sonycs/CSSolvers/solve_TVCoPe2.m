%% solve_TVCoPe2 : 
%{
Involves a slightly different decoder than solve_TVCoPe

Below we solve: 
min_x \lambda ||x||_1 + \lambda_2 || D_t x ||_1 + 1/2 ||A Phi^{-1} x-y||_2^2

-- Note: Difference operator D_t acts on the sparse signal x.
%}
...
L=5; fn='tennis.avi'; %fn='riksch1.avi';
...
% paramaters: gamma, lambda, maxItersCP
...
while  (iter <= maxItersCP)  %&& (normresrel > OptTol)
    ...
    % i=2
    z2 = v2 + gamma*diff(reshape(xhat,CS.Phi.N),1,3); % CHANGED
    % compute proximal mapping P_G of 1-norm operator with lambda2
    z2prox  = z2-(abs(z2) > lambda2) .*(z2 - lambda2.*sign(z2));
    v2 = v2 - z2 + z2prox + gamma*diff(reshape(yhat,CS.Phi.N),1,3); % CHANGED
    % end cases
    ...
    corr=diff_adjoint(z2prox); % CHANGED
