classdef CSSparseSignal_U <DC
    ...
    methods
        ...
        function obj=CSSparseSignal_U(hA,hPhi)
        ...
        function set_original(obj, hxorig, Nnew)
            ...
            % do not normalize in case we need original range of
            % intensities!
        ...
        function set_methodsRecon(obj)
            ...
            value5 = {'BP_\epsilon','BPDN','greedy','BPDN_{l1}','BPDN_{l2}',...
        ...
        function compute_QualityBound(obj)
            % ~() computes xopt at compression ctrafo
    ...
    methods (Access=protected)
        ...
        function s= PFstar_l2_Cons(obj,z,sig)
            ...
            if (isempty(obj.params.epsilon) ...
                    | obj.params.epsilon == 0)
                obj.params.epsilon=0.1;
            end         
            eps=obj.params.epsilon;
            res=norm(z-sig*obj.y);
            if  res > eps*sig
                s=(1-eps*sig/res)*(z-sig*obj.y);
            else
                s=zeros(size(z));
            end
        end
        ...
        function s= PFstar_l1_Cons(obj,z,sig)
            ...
            eps=obj.params.epsilon;
            PF=ProjectOntoTranslatedL1Ball(z/sig,obj.y,eps);
            s=z-sig*PF;
        end
    ...
    methods
        ...
        function graph_recon(obj, open_new)
            ...
            if ~obj.xorig.isemptydata
                ...
                if ~isempty(obj.solutionProps.quality.SSIM)
                    SSIM_str=[', SSIM=',stat2str(obj.solutionProps.quality.SSIM,'%3.2f','%3.0e')];
                end
                result_str=['PSNR=',stat2str(obj.solutionProps.quality.PSNR,'%3.1f','%3.1f'),...
                    SSIM_str];
            end
        ...
        function graph_opt(obj, open_new)
            ...
            resultOpt_str=[Phi_str,', PSNR=',stat2str(PSNR_opt,'%3.1f','%3.1f'),...
                SSIM_str,commentstr];
