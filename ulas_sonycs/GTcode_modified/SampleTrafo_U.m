classdef SampleTrafo_U < FrameTrafo
    ...
    methods
        ...
        function obj=SampleTrafo_U(signal)
        ...
        function yn= synthesize(obj,x)
            ...
            % SampleTrafo_U.synthesize uses dec instead of rec
        ...
        function xn= analyze(obj,cC)
            % yn=~(x) decompose x and sample at nodes
            % SampleTrafo_U.analyze uses rec instead of dec
...
%                 'local', 'yn is measurement vector');
    ...
    methods
        ...
        function nv=frame_dim(obj) % modified by ulas
            % n=~() dimension of Hilbert space
            % must be redefined for some subclasses
            
            log = ismember(properties(obj.ftrafo),'blocksize');
            
            if all(log==0) % no block measurement
                nv=obj.nodes.numel;
            else % block measurement
                nb=obj.ftrafo.numblock;
                ntot=nb(1)*nb(2);
                nv=ntot*obj.nodes.numel;
            end
        end
    end
