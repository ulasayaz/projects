classdef CompressiveSensing <CSSparseSignal
    % CS-methods to reconstruct a sparsifiable signal from an underdetermined measurement y= A*x.
    % In contrast to class CSSparseSignal the sparsifying
    ...
    %     --- measurement compression rate: 
                                            c=8; params=struct; params.w1=0.999; % 0.95;
    %     --- random nodes:                 
    ...
    %     simulate measurement and reconstruction: 
    ...
    % ------------------------------
        ...
        signal=Signal_Factory.make_CyclefromSignal2D(s2,videoflag,L,motionunit);
        Phi=Wavelet3D_mlab(signal);
        B=HartleyTrafo3(signal);
    %     -- Samling Nodes: 
        c=10; nodes=B.nodesDefault(c);
    ...
    %     yields for sampled ONS (SampleTrafo) a first iteration
    ...
    properties (SetAccess=protected)
        ...
        nodes  %@<SampledNodes> used to test SampleTrafo
    ...
    methods
        ...
        function obj=CompressiveSensing(hA,hPhi)
            ...
            obj = obj@CSSparseSignal(hA,hPhi);
        ...
        function set_APhi(obj,hA,hPhi)
            ...
            set_APhi@CSSparseSignal(obj,hA,hPhi);
        ...
        function sim_SampledONS(obj,Phi,B,nodes,SNR)
            % ~(Phi,B,nodes,[SNR]) simulate measurement and reconstruction for a sampled
        ...
        function set_nodesByMask(obj,mask)
            ...
            obj.requireOnly(isa(obj.A,'SampleTrafo'),'local',...
                'needs aSampleTrafo as encoder');
