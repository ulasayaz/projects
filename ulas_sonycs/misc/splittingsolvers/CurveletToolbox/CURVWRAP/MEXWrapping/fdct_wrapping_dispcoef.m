function img = fdct_wrapping_dispcoef(C, lev)

% fdct_wrapping_dispcoef - returns an image containing all the curvelet coefficients
%
% Inputs
% C   ... Curvelet coefficients (result of fdct_wrapping)
% lev ... #levels up from coarsest level to be shown (added GT 1/2014)
% Outputs
% img ...  Image containing all (or lev up from coaresest level)
%          the curvelet coefficients. The coefficents are rescaled so that
...
nbscales=min(lev,lev_max)+1;
  
  img = C{1}{1}; img = img/max(max(abs(img))); %normalize
  for sc=2:nbscales-1
    nd = floor(length(C{sc})/4);
    if nd==0
        % happens if finest==2: finest level is wavelet not curvelet (GT 1/2014)
        break;
    end
    wcnt = 0;
