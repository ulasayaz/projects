function [bandwidth,density,xmesh,cdf]=kde(data,params)
...
%  .bandwidth - choose non-automatic bandwidth  ( added by GT July 2014)
%  .title     - title for graphics
%   
% OUTPUTS:
...
if ~isfield(params,'n') || isempty(params.n)% if n is not supplied switch to the default
    params.n=2^14;
end
n=2^ceil(log2(params.n)); % round up n to the next power of 2;
if ~isfield(params,'MAX') || ~isfield(params,'MIN') %define the default  interval [MIN,MAX]
    minimum=min(data); maximum=max(data);
    Range=maximum-minimum;
    params.MIN=minimum-Range/10; params.MAX=maximum+Range/10;
end
...
if ~isfield(params,'open_new') || isempty(params.open_new)
    params.open_new=true;
end
...
if nargout==0
    ...
    plot(xmesh,density);
    title(['Gaussian kde ',params.title],'fontsize',12);
end
% for cdf estimation
if nargout>3
